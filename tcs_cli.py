...
def main():
    parser = argparse.ArgumentParser(description="TimeCodeSecurity (TCS) SAST Scanner CLI")
    parser.add_argument("target", nargs="?", default=".", help="Target file or directory to scan (default: current directory)")
    parser.add_argument("--format", choices=["json", "sarif"], default="json", help="Output format (json or sarif)")
    parser.add_argument("-o", "--output", help="Write output to specified file path instead of stdout")
    ...
    if args.output:
        print(f"[TCS CLI] Results saved to: {args.output} (Format: {args.format})", file=sys.stderr)
